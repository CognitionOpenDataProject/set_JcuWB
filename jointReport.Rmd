...
It's unclear from the paper, but While these look like they could be descriptive cell means in the paper may actually be model-derived marginal means. The cell means are not reported, but are shown in Figure 2, where they do seem to match up directly with the descriptive means we get here.
...
Note that this seems to match up pretty well with the results in df.object.prime_type.summary, although the values from which this graph is plotted are never directly reported in the paper, so it's impossible to run CompareValues.
...
anova.rs_neutral_included <- ezANOVA(
  df.tidy %>%
...
anova.rs_neutral_excluded <- ezANOVA(
  df.tidy %>%
...
obtainedValues_neutral_excluded_PrimeInteraction = filter(anova.rs_neutral_excluded$ANOVA, Effect == "prime_type:object")
obtainedValues_neutral_included_PrimeInteraction = filter(anova.rs_neutral_included$ANOVA, Effect == "prime_type:object")

## df numerator
#neutral included
compareValues(
  reportedValues_ownerPrimeInteraction$dfn,
  obtainedValues_neutral_included_PrimeInteraction$DFn
)

#neutral excluded
compareValues(
  reportedValues_ownerPrimeInteraction$dfn,
  obtainedValues_neutral_excluded_PrimeInteraction$DFn
)
...
compareValues(
  ...
  obtainedValues_neutral_included_PrimeInteraction$DFd
)

#neutral excluded
compareValues(
  reportedValues_ownerPrimeInteraction$dfd,
  obtainedValues_neutral_excluded_PrimeInteraction$DFd
)
...
compareValues(
  ...
  obtainedValues_neutral_included_PrimeInteraction$F
)

#neutral excluded
compareValues(
  reportedValues_ownerPrimeInteraction$f,
  obtainedValues_neutral_excluded_PrimeInteraction$F
)
...
compareValues(
  ...
  obtainedValues_neutral_included_PrimeInteraction$p, 
  isP = T
)
...
compareValues(
  reportedValues_ownerPrimeInteraction$p,
  obtainedValues_neutral_excluded_PrimeInteraction$p, 
  isP = T
)

``` 

It's unclear how they specified their ANCOVA model. They say they included all three levels of the prime_type (i.e. their "picture type") variable. If you run a model with all of these, however, the results are way off. For main effects, our DFd is off by 1 from theirs. For the main effect of gender, they report F(1,53), and we have F(1,54). However, for the interaction terms (e.g. the central analysis of interest, the interaction of prime_type and object), the problem is much worse: we have F(2, 108) and they report F(1, 53). Correspondingly, this leads to several numerical errors and, especially a decision error on the interaction, which is the central result of Exp 2 (they report p=0.01, we get p=0.13).

However, the simple effects they report seem to focus on the "self" and "other" level, but not the "neutral" level. This suggests that their ANCOVA may have excluded the "neutral" level. If you run an ANCOVA with this level excluded, you get DFd much more similar to theirs: (2,54) compared to their (2,53). You also get p-values much more similar to what they report, all over the place. E.g. the F-value of the interaction they report is 4.83, and the one derived here is 4.87. There are no longer any decision errors.

My conclusion is that this is very likely what they did, although it is not (as far as I can tell) what they report. I still can't understand where the difference of 1 in DFd comes from, though.

Note that they likely ran their ANCOVA in SPSS (which, unlike R, gives significance and F and p-values for the covariate of age, which they report in their paper), but specifying type=3 (which is supposed to mimic SPSS, vs default 2 in ezANOVA) doesn't affect the data.
...
No errors on the t-tests at all, except on the p-values. It seems like the authors report all their p-values as p = 0.01 (when in fact they are < 0.01). They say they use a Bonferroni correction (but don't say how many multiple comparisons they're correcting for), so the p-values they report are probably corrected. Note that they do not report applying a correction to the t-test in the neutralFocus cases (presumably because it's not significant anyway), so that leads the p-values to be a match in that case (further confirming that where they don't match, it's because of a correction being applied)

Note also that the way t-tests are described in the paper sounds more like one-sample comparisons of change in each condition to zero, rather than between conditions. They write, "children increased the value of their own toy... but not the experimenter’s toy". However, the t-statistics reported appear to be derived from comparison between conditions (i.e. the significant difference between children's increase in value of their own toy, relative to the experimenter's toy)
...
Note first of all, that the numbers of errors reported above are using the most generous interpretations of what the authors did (see above). If, for example, the main interaction between object and prime_type included all levels of both variables, there would be a decision error and even more major numerical errors.

Overall, I think this was a successful replication with several caveats. The data set has informative labels (though not the best structure for re-analysis, e.g., the "endowment_baseline" applies to both the experimenter_object and the child_object -- which is reported in the paper to be the same -- but to analyze the data, you need to duplicate that column).

However, the vast majority of our exact numerical calculations are off from the authors by varying degrees. These differences appear to be due to significant gaps and confusing/misleading statements in what the authors report relative to what they did. Specifically, what is described as a series of one-sample t-tests in the text are actually paired two-sample tests. It also appears (though it's harder to be sure here) that one of three levels of a variable is excluded in the main ANCOVA -- this causes the interaction between the two variables (one of the main results of paper) to be significant, as reported in the paper. With this level included, the interaction is not significant, leading to a decision error which would dramatically alter the theoretical conclusions of the paper. It may be possible to justify excluding this level from the analysis on theoretical grounds, but the authors do not report this exclusion in the text and do not provide such a justification.
