...
```{r}
df %>% filter(study == 1) %>% group_by(prime_type) %>% summarise(mean=mean(to.n(child_change), na.rm=TRUE))
df %>% filter(study == 1) %>% group_by(prime_type) %>% summarise(mean=mean(to.n(experimenter_change), na.rm=TRUE))
```
